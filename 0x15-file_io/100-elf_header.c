...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	elf_h = malloc(sizeof(Elf64_Ehdr));
	if (elf_h == NULL)
	{
		close_elf(op);
		dprintf(STDERR_FILENO, "Error: Can't read file %s\n", argv[1]);
	...
	rd = read(op, elf_h, sizeof(Elf64_Ehdr));
	if (rd == -1)
	{
		free(elf_h);
		close_elf(op);
	...
	check_if_elf(elf_h->e_ident);
	printf("ELF Header:\n");
	print_magic(elf_h->e_ident);
	print_class(elf_h->e_ident);
	print_data(elf_h->e_ident);
	print_version(elf_h->e_ident);
	print_osabi(elf_h->e_ident);
	print_abi(elf_h->e_ident);
	print_type(elf_h->e_type, elf_h->e_ident);
	print_entry(elf_h->e_entry, elf_h->e_ident);
